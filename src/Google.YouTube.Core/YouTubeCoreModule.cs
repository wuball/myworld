...
using Google.YouTube.Authorization.Ldap;
using Google.YouTube.Authorization.Roles;
using Google.YouTube.Authorization.Users;
using Google.YouTube.Chat;
using Google.YouTube.Configuration;
using Google.YouTube.DashboardCustomization.Definitions;
using Google.YouTube.Debugging;
using Google.YouTube.DynamicEntityProperties;
using Google.YouTube.Features;
using Google.YouTube.Friendships;
using Google.YouTube.Friendships.Cache;
using Google.YouTube.Localization;
using Google.YouTube.MultiTenancy;
using Google.YouTube.Net.Emailing;
using Google.YouTube.Notifications;
using Google.YouTube.WebHooks;
using Newtonsoft.Json;

namespace Google.YouTube
{
    [DependsOn(
        typeof(YouTubeCoreSharedModule),
        typeof(AbpZeroCoreModule),
        typeof(AbpZeroLdapModule),
        typeof(AbpAutoMapperModule),
        typeof(AbpAspNetZeroCoreModule),
        typeof(AbpMailKitModule))]
    public class YouTubeCoreModule : AbpModule
    {
        public override void PreInitialize()
        {
            ...
            YouTubeLocalizationConfigurer.Configure(Configuration.Localization);
            ...
            Configuration.MultiTenancy.IsEnabled = YouTubeConsts.MultiTenancyEnabled;
            ...
            Configuration.ReplaceService<IMailKitSmtpBuilder, YouTubeMailKitSmtpBuilder>(DependencyLifeStyle.Transient);
            ...
            Configuration.ReplaceService(typeof(IEmailSenderConfiguration), () =>
            {
                Configuration.IocManager.IocContainer.Register(
                    Component.For<IEmailSenderConfiguration, ISmtpEmailSenderConfiguration>()
                             .ImplementedBy<YouTubeSmtpEmailSenderConfiguration>()
                             .LifestyleTransient()
                );
            });
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(YouTubeCoreModule).GetAssembly());
        }
