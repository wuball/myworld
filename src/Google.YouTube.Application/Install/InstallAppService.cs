...
using Google.YouTube.Migrations.Seed.Host;
...
namespace Google.YouTube.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : YouTubeAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<YouTubeDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{YouTubeConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{YouTubeConsts.ConnectionStringName}", constring);
        }
